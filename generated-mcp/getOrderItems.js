...
module.exports = async function getOrderItems(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/business/orders/{OrderID}";
  for (const p of [{"name":"OrderID","type":"string","required":true,"description":"Order ID"}]){
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"order_token","type":"string","required":false,"description":"Base64 token from order for receiving information"}]){
    const v = args[q.name];
