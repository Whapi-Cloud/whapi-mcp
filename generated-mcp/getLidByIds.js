...
module.exports = async function getLidByIds(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/contacts/lids";
  for (const p of []){
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"ContactIDList","type":"array","required":true,"description":"Comma separated list of contact IDs"}]){
    const v = args[q.name];
