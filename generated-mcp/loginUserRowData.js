...
module.exports = async function loginUserRowData(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/users/login/rowdata";
  for (const p of []){
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"wakeup","type":"boolean","required":false,"default":true,"description":"If set to false, the channel will not launch"}]){
    const v = args[q.name];
