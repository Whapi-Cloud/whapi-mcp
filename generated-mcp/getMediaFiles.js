...
module.exports = async function getMediaFiles(args, env = process.env) {
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"count","type":"number","required":false,"default":100,"description":"Count of objects to return"},{"name":"offset","type":"number","required":false,"description":"Offset of objects to return"},{"name":"time_from","type":"number","required":false,"description":"Timestamp from which to get objects"},{"name":"time_to","type":"number","required":false,"description":"Timestamp up to which to get objects"},{"name":"sort","type":"string","required":false,"enum":["asc","desc"],"description":"Order for items in request"}]){
    const v = args[q.name];
