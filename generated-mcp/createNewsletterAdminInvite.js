...
module.exports = async function createNewsletterAdminInvite(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/newsletters/{NewsletterID}/invite/{ContactID}";
  for (const p of [{"name":"NewsletterID","type":"string","required":true,"description":"Newsletter ID"},{"name":"ContactID","type":"string","required":true,"description":"Contact ID"}]){
  ...
  if (method !== 'GET'){
    ...
    const bodyObj = {};
    if (args.hasOwnProperty('message')) bodyObj['message'] = args['message'];
