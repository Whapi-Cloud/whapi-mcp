...
module.exports = async function rejectCall(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/calls/{CallID}";
  for (const p of [{"name":"CallID","type":"string","required":true,"description":"Call ID"}]){
  ...
  if (method !== 'GET'){
    ...
    const bodyObj = {};
    if (args.hasOwnProperty('callFrom')) bodyObj['callFrom'] = args['callFrom'];
