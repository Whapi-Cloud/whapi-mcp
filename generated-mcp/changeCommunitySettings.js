...
module.exports = async function changeCommunitySettings(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/communities/{CommunityID}/settings";
  for (const p of [{"name":"CommunityID","type":"string","required":true,"description":"Community ID"}]){
  ...
  if (method !== 'GET'){
    ...
    const bodyObj = {};
    if (args.hasOwnProperty('setting')) bodyObj['setting'] = args['setting'];
    if (args.hasOwnProperty('policy')) bodyObj['policy'] = args['policy'];
