...
module.exports = async function recommendedNewsletter(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/newsletters/recommended";
  for (const p of []){
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"country_code","type":"string","required":false,"description":"Country for search"},{"name":"cursor","type":"string","required":false,"description":"Cursor to continue pagination"},{"name":"count","type":"number","required":false,"default":100,"description":"Count of objects to return"}]){
    const v = args[q.name];
