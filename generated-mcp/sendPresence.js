...
module.exports = async function sendPresence(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/presences/{EntryID}";
  for (const p of [{"name":"EntryID","type":"string","required":true,"description":"Contact or group id"}]){
  ...
  if (method !== 'GET'){
    ...
    const bodyObj = {};
    if (args.hasOwnProperty('presence')) bodyObj['presence'] = args['presence'];
    if (args.hasOwnProperty('delay')) bodyObj['delay'] = args['delay'];
