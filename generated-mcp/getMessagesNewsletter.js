...
module.exports = async function getMessagesNewsletter(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/newsletters/{NewsletterID}/messages";
  ...
  // Query string
  const queryPairs = [];
  for (const q of [{"name":"count","type":"number","required":false,"default":100,"description":"Count of objects to return"},{"name":"before","type":"number","required":false,"description":"Request messages before the specified one (see first and last)"},{"name":"after","type":"number","required":false,"description":"Request messages after the specified one (see first and last)"}]){
    const v = args[q.name];
