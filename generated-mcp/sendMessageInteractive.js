...
module.exports = async function sendMessageInteractive(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/messages/interactive";
  for (const p of []){
  ...
  if (method !== 'GET'){
    ...
    if (args.hasOwnProperty('header')) bodyObj['header'] = args['header'];
    if (args.hasOwnProperty('body')) bodyObj['body'] = args['body'];
    if (args.hasOwnProperty('footer')) bodyObj['footer'] = args['footer'];
    if (args.hasOwnProperty('action')) bodyObj['action'] = args['action'];
