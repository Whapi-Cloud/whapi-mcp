...
module.exports = async function editBusinessProfile(args, env = process.env) {
  // Build path with path params
  let pathTmpl = "/business";
  for (const p of []){
  ...
  if (method !== 'GET'){
    ...
    if (args.hasOwnProperty('email')) bodyObj['email'] = args['email'];
    if (args.hasOwnProperty('hours')) bodyObj['hours'] = args['hours'];
    if (args.hasOwnProperty('websites')) bodyObj['websites'] = args['websites'];
